...
const { generateKey } = require("../utils/aes");
            ...
            if (error) {
              ...
              signToken({ id: user._id }, (err, token) => {
                if (err) {
                  ...
                  res
                    .status(200)
                    .json({ status: "Email sent for verification" });
                }
              });
            }
          });
...
const isSignedUp = (req, res) => {
  User.exists({ email: req.query.email }, (err, result) => {
    if (err) res.status(500).json({ status: "Internal Server Error" });
    else if (!result) res.status(200).json({ registered: false });
    else
      User.findOneAndDelete(
        { email: req.query.email, verified: false },
        (err, doc) => {
          if (err) res.status(500).json({ status: "Internal Server Error" });
          else if (!doc) res.status(200).json({ registered: true });
...
const login = (req, res) => {
  User.findOne(
    ...
    "_id password services.sname",
    (err, user) => {
      if (err) res.status(500).json({ status: "Internal Server Error" });
      else if (user === null)
        res
          .status(401)
          .json({ status: "User account not registered, please signup" });
      else
        compare(req.body.password, user.password, (err, result) => {
          if (err) res.status(500).json({ status: "Internal Server Error" });
            ...
            signToken(
              ...
              HRINSEC // 1 hour sesstion time passed to generate jwt.
            );
          }
        });
    }
